/*
Developer: Seyed Hamed Moosavi
Website: MarketOwl.ir
Telegram: @Drmoo19
Email: moosavimaleki@gmail.com
*/

var msg = "";

var CodeBeCode = true;
var CodeBeCodePrvMin = 1;


var SafeFroosh = true;
var SafeFrooshMin = 2;//2min

var SafeKharid = true;
var PercentAbove = 83;//85% of positive range (85% of 5 => 4.25 , 85% of 3 => 2.55)

var power = ((ct).Buy_I_Volume * (ct).Sell_CountI) / ((ct).Sell_I_Volume * (ct).Buy_CountI);
var sarane_k = ((ct).Buy_I_Volume * (pc) / (ct).Buy_CountI) / 1e7;
var sarane_f = ((ct).Sell_I_Volume * (pc) / (ct).Sell_CountI) / 1e7;


//CodeBeCode\
if (
    CodeBeCode &&
    (((pl) != (tmax)) || ((pl) == (tmax) && (qd1) < 0.5 * (tvol))) &&
    (z) < 11e9
) {
    var new_cc_Sell_N_Volume = goa("snv", CodeBeCodePrvMin);

    var new_cc_Buy_I_Volume = goa("biv", CodeBeCodePrvMin);
    var new_cc_Buy_CountI = goa("bci", CodeBeCodePrvMin);

    var oldsnv = go("snv", CodeBeCodePrvMin);
    var X = (ct).Sell_N_Volume / (tvol) * 100;
    var Y = oldsnv / (go("tvol", CodeBeCodePrvMin)) * 100;

    if (

        new_cc_Sell_N_Volume > 0 &&
        new_cc_Buy_I_Volume > 0 &&
        (ct).Sell_N_Volume - oldsnv > 800e3 &&
        X - Y > 25 &&
        (new_cc_Buy_I_Volume * (pc) / new_cc_Buy_CountI) > 30e7 &&
        new_cc_Buy_I_Volume > 0.3 * new_cc_Sell_N_Volume
    ) {
        (cfield0) += " کد به کد ";
        ...
        msg = "#" + "<b>" +
            ...
            "\n🔄 #" + ar2fa((l18)) +
            "\n" + "قیمت: " + (plp) + "%" +
            "\n" + "قدرت: " + (formatNumber(power, 2)) + "" +
            "\n" + "سرانه: " + (formatNumber(sarane_k));
        sendOnce(msg, "cbc", 300, false)
        resp = true
    }
}
...
if (
    ...
    (pmin) == (pmax) &&
    current_min > 121 &&
    ((qo1) != (tmax) || ((qo1) == (tmax) && (qo1) < 1))
) {

    var last_qd11 = go("qd1", 1);
    var last_tvol1 = go("tvol", 1);
    ...
    if (
        ...
        msg = "#" + "<b>" +
            ...
            "</b>" +
            "\n⬇ #" + ar2fa((l18));
        sendOnce(msg, "arz", 300, false)
        resp = true
    }
}

var PishGoshayesh = true;
if (
    PishGoshayesh &&
    current_min < 120 &&
    current_min > 115 &&
    !((l30).indexOf("استان") != -1 && (cs) == 56) &&
    (pd1) == (tmin) &&
    (po1) == (tmin) &&
    (qd1) >= 0.1 * (qo1) &&
    ((qd1) / (zd1)) > 0.8 * ((qo1) / (zo1)) &&
    ((qd1) / (zd1)) * (tmin) > 18000000 &&
    ((qd1) / (zd1)) * (tmin) < 90e7 &&
    (qo1) > 500000
) {
    ...
    msg = "#" +
        "<b>" + "پیش_گشایش" + "</b>" +
        "\n🎌 #" + ar2fa((l18)) +
        "\n" + "حجم فروش: " + formatNumber((qo1), 0) +
        "\n" + "حجم خرید: " + formatNumber((qd1), 0);
...
if (
    SafeKharid &&
    (plp) >= ((((tmax) / (py)) - 1) * PercentAbove) &&
    ((ct).Buy_I_Volume / (ct).Buy_CountI) >= 1.3 * ((ct).Sell_I_Volume / (ct).Sell_CountI) &&
    ((ct).Buy_I_Volume / (ct).Buy_CountI) * (pl) > 180e6 &&
    (tvol) > 200e3 &&
    (((pl) != (tmax)) || ((pl) == (tmax) && (qd1) < 0.1 * (tvol)))
) {

    var last_pl2 = go("pl", 2);
    if (
        ...
        msg = "#" +
            "<b>" + 'آستانه_صف_خرید' + "</b>" +
            '\n💥 #' + ar2fa((l18)) +
            '\n' + 'قدرت خرید : ' + formatNumber(((ct).Buy_I_Volume * (ct).Sell_CountI) / ((ct).Sell_I_Volume * (ct).Buy_CountI), 2) +
            '\n' + 'سرانه خرید : ' + formatNumber((ct).Buy_I_Volume * (pc) / (ct).Buy_CountI / 1e7, 1)

        getInfo((l18), function (info) {
            if (info && info.month_avg && parseInt(info.month_avg) > 0) {
                msg += '\n' + 'ن.حجم: ' + formatNumber((tvol) / parseInt(info.month_avg), 1)

            }
            sendOnce(msg, "kh", 5 * 60)
            return true
        })
...
if (
    ...
    ((ct).Buy_I_Volume / (ct).Buy_CountI) * (pl) > 500e6 &&
    (tvol) > 400e3 &&
    (tvol) * (pc) > 40e9 &&
    (((pl) != (tmax)) || ((pl) == (tmax) && (qd1) < 0.1 * (tvol)))
) {
    ...
    if (
        ...
        var msgs = "#" + "<b>" +
            ...
            "\n💵 #" + ar2fa((l18)) +
            "\n" + "قیمت: " + (plp) + "%" +
            "\n" + "قدرت: " + formatNumber(((ct).Buy_I_Volume * (ct).Sell_CountI) / ((ct).Sell_I_Volume * (ct).Buy_CountI), 2) +
            "\n" + "سرانه: " + formatNumber((ct).Buy_I_Volume * (pc) / (ct).Buy_CountI / 1e7, 1);

        sendOnce(msgs, "ss", 5 * 60)
...
// kharid sangin
var Buy_I_Volume1 = go("biv", 1);
var Buy_CountI1 = go("bci", 1);
var Sell_I_Volume1 = go("siv", 1);
var Sell_CountI1 = go("sci", 1);
var saranekh1 = ((ct).Buy_I_Volume - Buy_I_Volume1) * (pl) / ((ct).Buy_CountI - Buy_CountI1);
var saranef1 = ((ct).Sell_I_Volume - Sell_I_Volume1) * (pl) / ((ct).Sell_CountI - Sell_CountI1);
if (
    ((pl) != (tmax)) &&
    (ct).Buy_CountI > Buy_CountI1
) {

    if (
        (((ct).Buy_CountI - Buy_CountI1) < 15 &&
            ((ct).Buy_I_Volume - Buy_I_Volume1) * (pl) / ((ct).Buy_CountI - Buy_CountI1) > 300e7 &&
            ((ct).Buy_I_Volume - Buy_I_Volume1) * (pl) > 10e9 &&
            saranekh1 > 3 * saranef1)
        ...
        var msgkhs = "#" + "<b>" +
            ...
            "</b>" +
            "\n💰 #" + ar2fa((l18)) +
            "\n" + "مقدار خرید: " + Math.floor(((ct).Buy_I_Volume - Buy_I_Volume1) * (pl) / 1e7) + " م.ت" +
            "\n" + "تعداد: " + Math.floor(((ct).Buy_CountI - Buy_CountI1)) + " کد" + "  |  " +
            "میانگین هر کد: " + Math.floor(((ct).Buy_I_Volume - Buy_I_Volume1) * (pl) / ((ct).Buy_CountI - Buy_CountI1) / 1e7) +
            " م.ت" +
            "\n" + "قیمت: " + (plp) + "%" +
            "\n" + "قدرت: " + formatNumber(((ct).Buy_I_Volume * (ct).Sell_CountI) / ((ct).Sell_I_Volume * (ct).Buy_CountI), 2) +
            "\n" + "سرانه: " + formatNumber((ct).Buy_I_Volume * (pc) / (ct).Buy_CountI / 1e7, 1);

        sendOnce(msgkhs, "khs", 2, false)
    ...
    if (
        ...
        (((ct).Buy_CountI - Buy_CountI1) > 14 &&
            ((ct).Buy_I_Volume - Buy_I_Volume1) * (pl) / ((ct).Buy_CountI - Buy_CountI1) > 100e7 &&
            ((ct).Buy_I_Volume - Buy_I_Volume1) * (pl) > 45e9 &&
            saranekh1 > 1.5 * saranef1)
...
if (
    Afzayesg &&
    current_min - AfzayesgMin > 120
) {
    var Buy_I_Volume5 = go("biv", AfzayesgMin);
    var Sell_I_Volume5 = go("siv", AfzayesgMin);
    var Buy_CountI5 = go("bci", AfzayesgMin);
    var Sell_CountI5 = go("sci", AfzayesgMin);

    var power5 = (Buy_I_Volume5 * Sell_CountI5) / (Sell_I_Volume5 * Buy_CountI5);


    if (
        power - power5 > 0.7 &&
        power > 1.2 * power5 &&
        ((tvol) / [is5]) * power > 1.2 * power5 &&
        (((pl) != (tmax)) || ((pl) == (tmax) && (qd1) < 0.1 * (tvol))) &&
        (tvol) > 400000
    ) {
        var power_ap_5 = (goa("biv", 5) / goa("bci", 5)) / (goa("siv", 5) / goa("sci", 5))
        var power_ap_1 = (goa("biv", 1) / goa("bci", 1)) / (goa("siv", 1) / goa("sci", 1))


        if ((power - power5 > 0.5 && power_ap_1 > power_ap_5 && Buy_I_Volume5 * (pl) > 200e7)) {
            (cfield0) += " افزایش قدرت ";
            ...
            msg = "#" + "<b>" +
                ...
                "\n" + "قیمت: " + (plp) + "%" +
                "\n" + "قدرت: " + (formatNumber(power, 2)) + "" +
                "\n" + "سرانه: " + (formatNumber(sarane_k, 0)) +
                "\n" + "افزایش قدرت: " + (formatNumber(power - power5, 2)) + "";
            sendOnce(msg, "afz2", 1 * 60, false)
            resp = true
...
var Range = true;
var RangeMin = 5;
var RangePercent = 3.5;
if (
    Range &&
    current_min - RangeMin > 120
) {
    var plp2 = go("plp", RangeMin, false);
    var vol_ap5 = goa("tvol", RangeMin);


    var power_ap_55 = (goa("biv", 5) / goa("bci", 5)) / (goa("siv", 5) / goa("sci", 5));

    if (
        plp2 !== false &&
        vol_ap5 * (pl) > 7e9 &&
        (tvol) * (pl) > 10e9 &&
        (plp) - plp2 > RangePercent &&
        power_ap_55 > 1
    ) {
        // var power_ap_11 = (goa("biv", 1) / goa("bci", 1)) / (goa("siv", 1) / goa("sci", 1))
        // var sarane_ap_55 = goa("biv", 5) * (pc) / goa("bci", 5) / 1e7;
        // var sarane_ap_11 = goa("biv", 1) * (pc) / goa("bci", 5) / 1e7;
        ...
        var msgrange = "#" + "<b>" +
            ...
            "\n" + "قیمت: " + (plp) + "%" +
            "\n" + "قدرت: " + (formatNumber(power, 2)) +
            "\n" + "سرانه: " + (formatNumber(sarane_k, 2));

        sendOnce(msgrange, "range", 1 * 60, false)
        resp = true
...
if (
    SafeFroosh &&
    current_min - SafeFrooshMin > 120 &&
    (pl) > 1000 &&
    (po1) == (tmin)
) {
    var old_qo1 = go("qo1", 1);
    var old_vol = go("tvol", 1);
    var Buy_I_Volume55 = go("biv", 1);
    var Buy_N_Volume55 = go("bnv", 1);

    if (

        (qo1) < (old_qo1) * (1 - (4 / 100)) &&
        ((tvol) - old_vol) > (2 / 200) * (old_qo1) &&
        (pmin) == (pmax) &&
        (old_qo1) > 100000
        ...
        var msgt = "#" + "<b>" +
            ...
            "\n" + "⚡" + w +
            " #" + ar2fa((l18)) +
            "\n" + "حجم صف فروش قبلی: " + formatNumber((old_qo1), 0) +
            "\n" + "حجم صف فروش فعلی: " + formatNumber((qo1), 0);


        sendOnce(msgt, "fo3", 5, false)
    ...
    }, {title: "شخصی", all: "", rowStyle: "", row: "", excel: {header: "", row: ""}}];
    ;
    ...
    function MarketWatchPlus() {
        var MarketWatchPlus = {
            ...
            DefaultSettings: {
                ...
                LoadInstStat: 0,
                LoadInstHistory: 0,
                CustomTemplate: {colNo: 10, fontSize: 12, rowHeight: 20, cols: [], all: "", rowStyle: "", row: ""}
            },
            UpdateCounter: 0,
            UpdateIgnore: 0,
            ...
            QueryWindowNewFilter: function () {
                mw.Settings.Filters.push({FilterCode: "", FilterName: "فیلتر شماره " + mw.Settings.Filters.length});
                mw.SaveParams()
            },
            ...
            SaveBasket: function () {
                $.ajax({
                    ...
                    crossDomain: true,
                    xhrFields: {withCredentials: true},
                    data: {t: "save", d: JSON.stringify(mw.Settings), dt: 0},
                    dataType: "html",
                    success: function (data) {
                        if (data == "login") {
                            var html = "<a style='text-decoration:underline' href='" + MembersSite() + "/userloginnew.aspx?http://www.tsetmc.com/loader.aspx?ParTree=15131F' target='_top'> برای استفاده از این امکان می بایست عضو شوید و با نام کاربری خود وارد سایت شوید. برای ثبت نام یا ورود به سیستم کلیک کنید</a>";
            ...
            SaveTemplate: function () {
                ...
                MWTemplates[MWTemplates.length - 1] = {
                    ...
                    row: TemplateRow,
                    excel: {header: ExcelHeader, row: ExcelRow}
                };
                mw.PrepareForMobile();
                mw.SaveParams();
                HideAllWindow()
            ...
            StartMarketWatch: function () {
                ...
                DrawRainbowTab({
                    ...
                    item: [{
                        ...
                        OnShowFunction: ""
                    }, {Title: "کلاس های خصوصی", RelatedElement: "#infoc", OnShowFunction: ""}]
                });
            ...
            UpdateMarketWatch: function () {
                ...
                $.ajax({
                    ...
                    cache: true,
                    data: {h: 5 * Math.floor(mw.heven / 5), r: 25 * Math.floor(mw.refid / 25)},
                    dataType: "html",
                    timeout: 10000,
                    error: function () {
                        if (typeof timer1 != "undefined") {
            ...
            LoadClientType: function () {
                ...
                $.ajax({
                    url: "tsev2/data/ClientTypeAll.aspx", cache: true, dataType: "text", success: function (data) {
                        if (data.length == 0) {
                            return
                        }
                        var rows = data.split(";");
            ...
            RenderData: function () {
                ...
                if (mw.userFullName) {
                    ...
                    $("#userFullName").html("");
                    $("#userLink").attr("href", "http://members.tsetmc.com/UserLoginnew.aspx?" + window.location.href).html("ورود")
                }
                mw.FirstTime = false
            },
            showNotification: function (title, body) {
                var notification = new Notification(title, {dir: "auto", lang: "", body: body, tag: "", icon: ""})
            },
